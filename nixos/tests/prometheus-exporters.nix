  ...
  */
  ...
  exporterTests = {
    ...
    podman = {
      ...
      exporterTest = ''
        wait_for_unit("prometheus-podman-exporter.service", timeout=15)
        succeed("curl -sSf http://localhost:9882/metrics | grep 'prometheus_podman_exporter_build_info{.*} 1'", timeout=15)
      '';
        # wait_for_open_port(9882) -- no timeout
        # succeed("systemctl show prometheus-podman-exporter.service")
        # wait_for_unit("prometheus-podman-exporter.service", timeout=15)
        # succeed("curl -sSf http://localhost:9882/metrics | grep 'prometheus_podman_exporter_build_info{.*} 1'", timeout=15)
        # wait_for_open_port(9882) -- no timeout
    };
